...
BEGIN
  ...
  v_base_details := jsonb_build_object(
    'cpf', regexp_replace(COALESCE(p_cpf,''), '^(\d{3})(\d{3})(\d{3})(\d{2})$', '***.\2.\3-**'),
  ...
  IF v_user_id IS NULL THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL, NULL,
  ...
  -- Idempotência por conversation_id: somente para retries imediatos do n8n (janela curta de 3 min).
  -- Conversation_id se repete ao longo do tempo (mesma conversa Chatwoot), então NÃO pode ser idempotência perpétua.
  IF p_conversation_id IS NOT NULL THEN
    ...
    WHERE br.conversation_id = p_conversation_id
      AND br.created_at > now() - interval '3 minutes'
  ...
  IF v_user_status = 'demitido' THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL, v_user_id,
  ...
  IF v_existing_protocol IS NOT NULL THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL, v_user_id,
      v_base_details || jsonb_build_object('reason','duplicate_open_request','message','Colaborador já possui solicitação aberta','existing_protocol', v_existing_protocol));
    RETURN jsonb_build_object(
      'success', false,
  ...
  IF v_required_days IS NOT NULL THEN
    IF v_admission_date_text IS NULL OR trim(v_admission_date_text) = '' THEN
      PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL, v_user_id,
        v_base_details || jsonb_build_object('reason','missing_admission_date','message','Colaborador não possui data de admissão cadastrada','benefit_type', v_benefit_type, 'benefit_label', v_benefit_label));
    ...
    IF v_days_since_admission < v_required_days THEN
      PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL, v_user_id,
        v_base_details || jsonb_build_object('reason','insufficient_admission_time','benefit_type', v_benefit_type, 'benefit_label', v_benefit_label, 'required_days', v_required_days, 'current_days', v_days_since_admission));
      RETURN jsonb_build_object(
        'success', false,
  ...
  IF p_protocol IS NOT NULL AND p_protocol != '' THEN
    ...
    IF EXISTS (SELECT 1 FROM benefit_requests WHERE protocol = v_final_protocol) THEN
      PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL, v_user_id,
        v_base_details || jsonb_build_object('reason','duplicate_protocol','protocol', v_final_protocol));
  ...
  VALUES (v_user_id, v_final_protocol, v_benefit_type, 'aberta',
    jsonb_build_object('source', 'bot', 'original_text', p_benefit_text, 'cpf', p_cpf),
