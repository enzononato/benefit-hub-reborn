...
BEGIN
  ...
  v_cpf_clean := replace(replace(COALESCE(p_cpf, ''), '.', ''), '-', '');
  -- mascarar CPF: mantém dígitos centrais, esconde extremos (ex: ***.456.789-**)
  IF length(v_cpf_clean) = 11 THEN
    v_cpf_masked := '***.' || substr(v_cpf_clean, 4, 3) || '.' || substr(v_cpf_clean, 7, 3) || '-**';
  ELSE
    v_cpf_masked := COALESCE(p_cpf, '');
  END IF;

  v_base_details := jsonb_build_object(
    'cpf', v_cpf_masked,
    'benefit_text', p_benefit_text,
  ...
  FROM profiles p
  WHERE replace(replace(p.cpf, '.', ''), '-', '') = v_cpf_clean
  LIMIT 1;

  IF v_user_id IS NULL THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
      v_base_details || jsonb_build_object('reason', 'user_not_found', 'message', 'Colaborador não encontrado com o CPF informado', 'name_provided', p_name),
      NULL);
    RETURN jsonb_build_object('success', false, 'error', 'user_not_found', 'message', 'Colaborador não encontrado com o CPF informado');
  END IF;

  -- Idempotência: replay de sucesso, não loga rejeição
  ...
  IF v_user_status = 'demitido' THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
      v_base_details || jsonb_build_object('reason', 'user_inactive', 'message', 'Colaborador com status demitido não pode criar solicitações', 'user_status', v_user_status),
      v_user_id);
    RETURN jsonb_build_object('success', false, 'error', 'user_inactive', 'message', 'Colaborador com status demitido não pode criar solicitações');
  END IF;

  IF v_user_status = 'afastado' THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
      v_base_details || jsonb_build_object('reason', 'user_on_leave', 'message', 'Colaborador afastado não pode criar solicitações', 'user_status', v_user_status),
      v_user_id);
    RETURN jsonb_build_object('success', false, 'error', 'user_on_leave', 'message', 'Colaborador afastado não pode criar solicitações');
  END IF;

  IF v_user_status = 'ferias' THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
      v_base_details || jsonb_build_object('reason', 'user_on_vacation', 'message', 'Colaborador de férias não pode criar solicitações', 'user_status', v_user_status),
      v_user_id);
    RETURN jsonb_build_object('success', false, 'error', 'user_on_vacation', 'message', 'Colaborador de férias não pode criar solicitações');
  END IF;

  IF v_user_status = 'limite_excedido' THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
      v_base_details || jsonb_build_object('reason', 'user_limit_exceeded', 'message', 'Limite excedido, peça novamente no mês seguinte', 'user_status', v_user_status),
      v_user_id);
    RETURN jsonb_build_object('success', false, 'error', 'user_limit_exceeded', 'message', 'Limite excedido, peça novamente no mês seguinte');
  END IF;
  ...
  IF v_existing_protocol IS NOT NULL THEN
    PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
      v_base_details || jsonb_build_object('reason', 'duplicate_open_request', 'message', 'Já possui solicitação em aberto', 'existing_protocol', v_existing_protocol),
      v_user_id);
    RETURN jsonb_build_object(
      'success', false,
      'error', 'duplicate_open_request',
      'message', 'Você já possui uma solicitação em aberto (protocolo ' || v_existing_protocol || '). Aguarde o atendimento antes de abrir outra.',
      'existing_protocol', v_existing_protocol
    );
  END IF;
  ...
  CASE v_lower_text
    ...
    WHEN 'solides' THEN v_benefit_type := 'solides';
  ...
  IF v_required_days IS NOT NULL THEN
    IF v_admission_date_text IS NULL OR trim(v_admission_date_text) = '' THEN
      PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
        v_base_details || jsonb_build_object('reason', 'missing_admission_date', 'message', 'Colaborador sem data de admissão', 'benefit_type', v_benefit_type, 'benefit_label', v_benefit_label),
        v_user_id);
      RETURN jsonb_build_object('success', false, 'error', 'missing_admission_date', 'message', 'Colaborador não possui data de admissão cadastrada. Necessário para solicitar convênios.');
    END IF;
    ...
    BEGIN
      ...
      ELSE
        PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
          v_base_details || jsonb_build_object('reason', 'missing_admission_date', 'message', 'Data de admissão em formato inválido', 'benefit_type', v_benefit_type, 'admission_raw', v_admission_date_text),
          v_user_id);
        RETURN jsonb_build_object('success', false, 'error', 'missing_admission_date', 'message', 'Data de admissão em formato inválido. Necessário para solicitar convênios.');
      END IF;
    EXCEPTION WHEN OTHERS THEN
      PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
        v_base_details || jsonb_build_object('reason', 'missing_admission_date', 'message', 'Data de admissão inválida (exception)', 'benefit_type', v_benefit_type, 'admission_raw', v_admission_date_text),
        v_user_id);
      RETURN jsonb_build_object('success', false, 'error', 'missing_admission_date', 'message', 'Data de admissão em formato inválido. Necessário para solicitar convênios.');
    END;
    ...
    IF v_days_since_admission < v_required_days THEN
      PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
        v_base_details || jsonb_build_object(
          'reason', 'insufficient_admission_time',
          'message', 'Tempo de admissão insuficiente',
          'benefit_type', v_benefit_type,
  ...
  IF p_protocol IS NOT NULL AND p_protocol != '' THEN
    ...
    IF EXISTS (SELECT 1 FROM benefit_requests WHERE protocol = v_final_protocol) THEN
      PERFORM create_system_log('bot_request_rejected', 'benefit_request_attempt', NULL,
        v_base_details || jsonb_build_object('reason', 'duplicate_protocol', 'message', 'Protocolo já existe', 'protocol', v_final_protocol),
        v_user_id);
      RETURN jsonb_build_object('success', false, 'error', 'duplicate_protocol', 'message', 'Protocolo já existe: ' || v_final_protocol);
    END IF;
  ELSE
  ...
  INSERT INTO benefit_requests (user_id, protocol, benefit_type, status, details, whatsapp_jid, account_id, conversation_id)
  VALUES (v_user_id, v_final_protocol, v_benefit_type, 'aberta',
  ...
  RETURN jsonb_build_object('success', true, 'request_id', v_request_id, 'protocol', v_final_protocol, 'benefit_type', v_benefit_type, 'user_name', v_full_name);
END;
$function$;
