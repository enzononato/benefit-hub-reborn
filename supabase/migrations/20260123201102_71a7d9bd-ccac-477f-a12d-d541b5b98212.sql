-- Dropar a função existente para recriar com nova assinatura
DROP FUNCTION IF EXISTS public.create_request_from_bot(text, text, text, text, text, bigint, bigint);

-- Adicionar novos valores ao enum benefit_type para categoria "Outros"
ALTER TYPE benefit_type ADD VALUE IF NOT EXISTS 'listagem_funcionarios';
ALTER TYPE benefit_type ADD VALUE IF NOT EXISTS 'listagem_aniversariantes';
ALTER TYPE benefit_type ADD VALUE IF NOT EXISTS 'listagem_dependentes';
ALTER TYPE benefit_type ADD VALUE IF NOT EXISTS 'listagem_pdcs';
ALTER TYPE benefit_type ADD VALUE IF NOT EXISTS 'informacoes_diversas';

-- Recriar a função com mapeamento atualizado
